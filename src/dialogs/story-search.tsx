...
export const StorySearchDialog: React.FC<StorySearchDialogProps> = props => {
	...
	function setScope(scope: SearchScope) {
		patchProps({flags: {...flags, searchScope: scope, includePassageNames: scope !== 'text'}});
	}

	const currentScope: SearchScope = flags.searchScope
			...
			<div className="search-flags">
				<div className="search-scope">
					<span className="search-scope-label">
						{t('dialogs.storySearch.searchIn')}
					</span>
					{(['text', 'names', 'both'] as SearchScope[]).map(s => (
						<button
							key={s}
							className={currentScope === s ? 'active' : ''}
							onClick={() => setScope(s)}
							type="button"
						>
							{t(`dialogs.storySearch.scope.${s}`)}
						</button>
					))}
				</div>
				<CheckboxButton

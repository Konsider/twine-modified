...
export type PassageDisplayMode = 'auto' | 'titles' | 'full';
...
export const PassageMap: React.FC<PassageMapProps> = props => {
	...
	const resolveCompact = React.useCallback(
		(z: number) => {
			if (displayMode === 'titles') return true;
			if (displayMode === 'full') return false;
			return z <= compactCardZoom;
		},
		[displayMode]
	);

	const [compactCards, setCompactCards] = React.useState(
		resolveCompact(visibleZoom)
	);
	...
	React.useEffect(() => {
		if (zoom === visibleZoom) {
			setCompactCards(resolveCompact(zoom));
		}
	}, [visibleZoom, zoom, resolveCompact]);

	// Respond immediately when displayMode is forced (not auto).
	React.useEffect(() => {
		if (displayMode !== 'auto') {
			setCompactCards(displayMode === 'titles');
		}
	}, [displayMode]);

...
export const reducer: React.Reducer<PrefsState, PrefsAction> = (
	...
	switch (action.type) {
		...
		case 'repair': {
			...
			// Custom version migrations. These run once based on version number.
			const currentMigration = state.customMigrationVersion || 0;

			if (currentMigration < 1) {
				// Migration 1: sort story list by date, use SugarCube.
				changes.storyListSort = 'date';
				changes.customMigrationVersion = 1;
			}

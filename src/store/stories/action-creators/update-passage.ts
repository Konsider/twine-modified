...
import {storyWithId, passageWithId} from '../getters';
...
export function updatePassage(
	...
	return (dispatch, getState) => {
		...
		if (!options.dontUpdateOthers && props.text) {
			...
			// If this passage is marked as an ending but now has outgoing links,
			// auto-remove the end flag.

			const updatedPassage = passageWithId(getState(), story.id, passage.id);

			if (updatedPassage.end && parseLinks(props.text, true).length > 0) {
				dispatch({
					props: {end: false},
					type: 'updatePassage',
					passageId: passage.id,

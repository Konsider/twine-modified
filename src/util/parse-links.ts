...
/**
 * Resolves the target passage name from the raw inner content of a
 * `[[…]]` link tag (i.e. with the enclosing brackets already stripped).
 *
 * Handles `->`, `<-`, `|` separators and `][` setter blocks.
 */
export function resolveLink(inner: string): string {
	const target = extractLink(removeSetters(inner) ?? inner);

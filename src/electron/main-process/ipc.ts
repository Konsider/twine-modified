...
export function initIpc() {
	...
	ipcMain.handle(
		...
		async (
			...
			const extension = defaultFilename.split('.').pop() || 'html';
			const filterName =
				extension === 'twee' ? 'Twee Files' : 'HTML Files';
	...
	// Passage image handlers.

	ipcMain.handle(
		'load-image-manifest',
		async (_event, storyIfid: string) => {
			try {
				return await loadImageManifest(storyIfid);
			} catch (error) {
	...
	ipcMain.handle(
		...
		async (
			...
			try {
				...
				console.warn(`Could not save passage image: ${error}`);
				return null;
			}
		}
	);
	...
	ipcMain.handle(
		...
		async (_event, storyIfid: string, passageName: string) => {
			try {
				await deletePassageImage(storyIfid, passageName);
				return true;
			} catch (error) {
				console.warn(`Could not delete passage image: ${error}`);
				return false;
			}
		}
	);
	...
	ipcMain.handle(
		...
		async (_event, storyIfid: string) => {
			try {
				await deleteStoryImages(storyIfid);
				return true;
			} catch (error) {
				console.warn(`Could not delete story images: ${error}`);
				return false;
			}
		}
	);
	...
	ipcMain.handle(
		...
		async (
			...
			try {
				await renamePassageImage(storyIfid, oldPassageName, newPassageName);
				return true;
			} catch (error) {
				console.warn(`Could not rename passage image: ${error}`);
				return false;
			}
		}
	);

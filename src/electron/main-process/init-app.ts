...
import {resolveImagePath} from './passage-images';
...
export async function initApp() {
	try {
		...
		// Register the handler for the twine-image:// protocol.
		// URL format: twine-image://{storyIfid}/{filename}

		protocol.handle('twine-image', (request) => {
			try {
				const url = new URL(request.url);
				const storyIfid = url.hostname;
				const filename = decodeURIComponent(url.pathname.replace(/^\//, ''));
				const filePath = resolveImagePath(storyIfid, filename);

				return net.fetch(`file://${filePath}`);
			} catch (error) {
				console.warn('Failed to resolve twine-image URL:', request.url, error);

...
export const BuildActions: React.FC<BuildActionsProps> = ({story}) => {
	...
	async function handlePublishFile() {
		...
		try {
			await saveHtml(await publishStory(story.id), storyFileName(story), prefs.defaultSaveDirectory);
		} catch (error) {
	...
	async function handleExportAsTwee() {
		...
		await saveTwee(storyToTwee(story), storyFileName(story, '.twee'), prefs.defaultSaveDirectory);
	}

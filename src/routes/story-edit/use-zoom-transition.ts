...
import {consumeWheelZoomFlag} from './use-zoom-wheel';
...
export function useZoomTransition(
	...
	React.useEffect(() => {
		if (!transition.current && current !== target && scrollTarget) {
			// If the zoom change came from the mouse wheel, snap instantly.
			// The wheel handler already set the DOM transform and scroll
			// position synchronously, so animating would fight with it.
			// Also snap for small changes (e.g. rapid-fire wheel events
			// that accumulate).

			if (consumeWheelZoomFlag()) {
				setCurrent(target);
				return;
			}

			// Animate larger changes from toolbar buttons.
			transition.current = {
				scrollTarget,
				elapsed: 0,
